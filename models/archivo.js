...
Archivo.findByIdOrdentarea = (idOrdentarea, created_by, next) => {
    if( !connection )
    ...
    let keys = [];
    if (created_by) {
        query = 'SELECT archivo.*, _tarea.nombre as ordentarea_ordentarea_idordentarea FROM archivo INNER JOIN ordentarea as _ordentarea_idordentarea ON _ordentarea_idordentarea.idordentarea = archivo.ordentarea_idordentarea INNER JOIN tarea as _tarea ON _tarea.idtarea = _ordentarea_idordentarea.tarea_idtarea  WHERE archivo.ordentarea_idordentarea = ? AND archivo.created_by = ? HAVING archivo.baja IS NULL OR archivo.baja = false';
        keys = [idOrdentarea, created_by];
    } else {
        query = 'SELECT archivo.*, _tarea.nombre as ordentarea_ordentarea_idordentarea FROM archivo INNER JOIN ordentarea as _ordentarea_idordentarea ON _ordentarea_idordentarea.idordentarea = archivo.ordentarea_idordentarea INNER JOIN tarea as _tarea ON _tarea.idtarea = _ordentarea_idordentarea.tarea_idtarea  WHERE archivo.ordentarea_idordentarea = ? HAVING archivo.baja IS NULL OR archivo.baja = false';
        keys = [idOrdentarea];
    }
...
Archivo.all = (created_by, next) => {
    ...
    let keys = [];
    if (created_by) {
        query = 'SELECT archivo.*, _tarea.nombre as ordentarea_ordentarea_idordentarea FROM archivo INNER JOIN ordentarea as _ordentarea_idordentarea ON _ordentarea_idordentarea.idordentarea = archivo.ordentarea_idordentarea INNER JOIN tarea as _tarea ON _tarea.idtarea = _ordentarea_idordentarea.tarea_idtarea  WHERE archivo.created_by = ? HAVING archivo.baja IS NULL OR archivo.baja = false';
        keys = [created_by];
    } else {
        query = 'SELECT archivo.*, _tarea.nombre as ordentarea_ordentarea_idordentarea FROM archivo INNER JOIN ordentarea as _ordentarea_idordentarea ON _ordentarea_idordentarea.idordentarea = archivo.ordentarea_idordentarea INNER JOIN tarea as _tarea ON _tarea.idtarea = _ordentarea_idordentarea.tarea_idtarea  HAVING archivo.baja IS NULL OR archivo.baja = false';
        keys = [];
    }
